...
public class BitmapText {
	...
	public void renderString(String text, int size) {
		...
		for (int i = 0; i < len; i++) {
			...
			GL11.glTexCoord2f(dx + (charCode[1] + 1) * fw, (charCode[0] + 1)
					* fh);
			GL11.glVertex3f(charPos[0] + ff * size, charPos[1], 1);
			GL11.glTexCoord2f(dx + (charCode[1] + 1) * fw, charCode[0] * fh);
			GL11.glVertex3f(charPos[0] + ff * size, charPos[1] + size, 1);
			GL11.glTexCoord2f(dx + charCode[1] * fw, charCode[0] * fh);
			GL11.glVertex3f(charPos[0], charPos[1] + size, 1);
			charPos[0] += ff * size;
		}
		GL11.glEnd();
	}
