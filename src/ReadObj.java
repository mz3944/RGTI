...
public class ReadObj {
		...
		try {
			...
			while ((s = dis.readLine()) != null) {
				...
				for (int i = 0; i < niz.length; i++) {
					if (niz[0].equals("v") || niz[0].equals("vn")
							|| niz[0].equals("vt") || niz[0].equals("f")) {
						if (niz[0].equals("v")) {
			...
			rearrange(vertices, normals, textures, vertexIndices,
	...
	private void rearrange(List<Float> vertices, List<Float> normals,
			List<Float> textures, List<Integer> vertexIndices,
		...
		for (int i = 0; i < vertexIndices.size(); i++) {
			...
			verticesRearranged[i * 3 + 1] = vertices
					.get(vertexIndices.get(i) * 3 + 1);
			verticesRearranged[i * 3 + 2] = vertices
					.get(vertexIndices.get(i) * 3 + 2);

			normalsRearranged[i * 3] = normals.get(normalIndices.get(i) * 3);
			normalsRearranged[i * 3 + 1] = normals
					.get(normalIndices.get(i) * 3 + 1);
			normalsRearranged[i * 3 + 2] = normals
					.get(normalIndices.get(i) * 3 + 2);

			texturesRearranged[i * 2] = textures.get(textureIndices.get(i) * 2);
			texturesRearranged[i * 2 + 1] = textures
					.get(textureIndices.get(i) * 2 + 1);
		}
	}
}