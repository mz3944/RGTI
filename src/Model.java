...
public class Model {
	...
	void initializeModels(){	
		...
		m_Textures = Texture.loadTextures2D(Arrays.copyOf(objTex.toArray(), objTex.toArray().length, String[].class));
		...
		numOfPoints = bufferSize;//System.out.println(((int)(bufferSize*(float)2/3)));
		bufferSize += bufferSize + (bufferSize*(float)2/3); // se za teksture in normale!
	...
	//  Creating buffers for the actual data													
	    verticesAndNormalsAndTextureBuffer = ByteBuffer.allocateDirect(bufferSize * BYTE_SIZE).order(ByteOrder.nativeOrder()).asFloatBuffer();
	    for(int i = 0; i < numOfObjs; i++) 
	...
	public void render3D() {
		...
		for(int i = 0; i < modelsPerObjs.size(); i++) {	
			for(int j = 0; j < modelsPerObjs.get(i); j++) {		
				...
				GL11.glPopMatrix();
				System.out.println(i + "  " + objTex.get(i));
...
//	}
	void renderModel(int startOffset, int length){
	    ...
	    GL11.glTexCoordPointer(TEXTURE_DIMENSION, GL_FLOAT, 0, ((numOfPoints*2) * BYTE_SIZE));
	    
	    //pred tem mormo klicat SUB-e!!!
	    //  drawing triangles
	    GL11.glDrawArrays(GL_TRIANGLES, startOffset*2, length); //*2 mogoce zato, ker so 16bitni podatki, in se tm doloc zamik v bytih, length je pa smao stevilo podatkov, torej stevilo 16bitnih podatkov, kar pomeni dvakrat mansa cifra...
